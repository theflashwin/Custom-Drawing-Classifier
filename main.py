...
class DrawingClassifier:
    ...
    def __init__(self):
        ...
        self.clf = None # Model you use for training and prediction
        self.proj_name = None # Directory in which test cases are stored
    ...
    def classes_prompt(self):
        ...
        if os.path.exists(self.proj_name):
            try:
                ...
                tkinter.messagebox.showerror(f"{self.proj_name}", f"The directory you specified exists already, but does not have a data file: {self.proj_name}_data.pickle", parent = self.root)
        else:
    ...
    def init_gui(self):
        ...
        train_btn = Button(btn_frame, text="Train", command = self.train_model)
    ...
    def clear(self):
        self.canvas.delete("all") # clears the entire canvas
        self.draw.rectangle([0, 0, 1000, 1000], fill="white") # the image we drew is not cleared, this clears it

    def train_model(self):
        img_list = np.array([]) # representing the pixels of the image
        class_list = np.array([]) # representing the classes (1, 2, 3)
        ...
        tkinter.messagebox.showinfo(f"{self.proj_name}", "Model sucessfully trained", parent = self.root)
    ...
    def predict(self):
        ...
        try:
            ...
            tkinter.messagebox.showerror(f"{self.proj_name}", "Make sure you train the model before trying to predict anything!", parent=self.root)
        
        if prediction[0] == 1:
            tkinter.messagebox.showinfo(f"{self.proj_name}", f"The drawing is probably a {self.class1}", parent=self.root)

        elif prediction[0] == 2:
            tkinter.messagebox.showinfo(f"{self.proj_name}", f"The drawing is probably a {self.class2}", parent=self.root)

        elif prediction[0] == 3:
            tkinter.messagebox.showinfo(f"{self.proj_name}", f"The drawing is probably a {self.class3}", parent=self.root)
    ...
    def save_model(self):
        ...
        tkinter.messagebox.showinfo(f"{self.proj_name}", "Model Sucessfully Saved", parent=self.root)
    
    def load_model(self):
        file_path = filedialog.askopenfilename()
        with open(file_path, "rb") as f:
            self.clf = pickle.load(f)
        tkinter.messagebox.showinfo(f"{self.proj_name}", "Model Sucessfully Loaded", parent=self.root)
    
    def save_everything(self):
        data = {"c1": self.class1, "c2": self.class2, "c3": self.class3, "c1c": self.class1_counter, "c2c": self.class2_counter, "c3c": self.class3_counter,
                "clf": self.clf, "pname": self.proj_name}
        with open(f"{self.proj_name}/{self.proj_name}_data.pickle", "wb") as f:
            pickle.dump(data, f)
        tkinter.messagebox.showinfo(f"{self.proj_name}", "Project Sucessfully Saved", parent=self.root)
